...
class _ResultPageContentState extends State<_ResultPageContent> {
  // Local list to track completed reports for incremental updates
  ...
  void initState() {
    super.initState();
    // Initialize with parent reports (all completed before navigation)
    _reports = List.from(widget.reports);
    // Mark loaded if parent provided all reports
    _allReportsLoaded = _reports.length == widget.reportFutures.length;
    // Initialize pred labels from any existing reports
    _predLabels.addAll(_reports.map((r) => r.pred_label));
    // Listen for incoming report futures to update reports and pred labels
    for (final future in widget.reportFutures.skip(_reports.length)) {
      future.then((report) {
        if (mounted) {
          setState(() {
            _reports.add(report);
            _predLabels.add(report.pred_label);
            if (_reports.length == widget.reportFutures.length) {
  ...
  Future<void> _restoreReportsIfNeeded() async {
    // チャットが終了している && レポート作成中でない場合のみrestore
    if (_skipRestore) return;
    if (_reports.isNotEmpty || _allReportsLoaded) return;
    // reportFuturesが空でない場合（=まだFutureでレポート作成中）はrestoreしない
    if (widget.reportFutures.isNotEmpty) return;
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    final userId = user.uid;
            ...
            child: Column(
              ...
              children: [
                ...
                _reports.length > i
                    ...
                    : FutureBuilder<JudgeAndReport>(
                        future: widget.reportFutures.length > i ? widget.reportFutures[i] : null,
                        builder: (context, snapshot) {
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            ...
                            return MarkdownBody(data: report.report);
                          } else {
